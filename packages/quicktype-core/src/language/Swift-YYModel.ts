...
export const swiftYYModelOptions = {
    ...
    mutableProperties: new BooleanOption("mutable-properties", "Use var instead of let for object properties", true),
    ...
    dense: new EnumOption(
        ...
        "normal",
        "secondary"
    ),// density 表示声明的变量如果类型一样就放在一行，这里我们选择 normal
...
export class SwiftYYModelTargetLanguage extends TargetLanguage {
    constructor() {
        super("Swift-YYModel", ["swift-yymodel", "swift4-yymodel"], "swift");
    }

    protected getOptions(): Option<any>[] {
        return [
            swiftYYModelOptions.justTypes,
            swiftYYModelOptions.useClasses,
            swiftYYModelOptions.dense,
            swiftYYModelOptions.convenienceInitializers,
            swiftYYModelOptions.explicitCodingKeys,
            swiftYYModelOptions.accessLevel,
            swiftYYModelOptions.alamofire,
            swiftYYModelOptions.linux,
            swiftYYModelOptions.namedTypePrefix,
            swiftYYModelOptions.protocol,
            swiftYYModelOptions.acronymStyle,
            swiftYYModelOptions.objcSupport,
            swiftYYModelOptions.optionalEnums,
            swiftYYModelOptions.sendable,
            swiftYYModelOptions.swift5Support,
            swiftYYModelOptions.multiFileOutput,
            swiftYYModelOptions.mutableProperties
        ];
    }
    ...
    protected makeRenderer(renderContext: RenderContext, untypedOptionValues: { [name: string]: any }): SwiftYYModelRenderer {
        return new SwiftYYModelRenderer(this, renderContext, getOptionValues(swiftYYModelOptions, untypedOptionValues));
    }
...
export class SwiftYYModelRenderer extends ConvenienceRenderer {
    ...
    constructor(
        ...
        private readonly _options: OptionValues<typeof swiftYYModelOptions>
    ) {
    ...
    private swiftYYModelPropertyType(p: ClassProperty): Sourcelike {
        return [this.swiftType(p.type, true, true), this.swiftYYModelTypeDefaultValue(p.type, true)];
    }
    ...
    private swiftYYModelTypeDefaultValue(t: Type, withIssues = false): Sourcelike {
        return matchType<Sourcelike>(
            t,
            _anyType => "?",
            _nullType => "?",
    ...
    private propertyYYModelLinesDefinition(name: Name, parameter: ClassProperty): Sourcelike {
        const useMutableProperties = this._options.mutableProperties;
        return [
            this.accessLevel,
            useMutableProperties ? "var " : "let ",
            name,
            ": ",
            this.swiftYYModelPropertyType(parameter)
        ];
    }
    ...
    private renderClassDefinition(c: ClassType, className: Name): void {
        ...
        this.emitBlockWithAccess([structOrClass, " ", className, this.getProtocolString(structOrClass)], () => {
            ...
            this.forEachClassProperty(c, "none", (name, jsonName, p) => {
                ...
                const propertyLines = this.propertyYYModelLinesDefinition(name, p);
            ...
            // YYModel 协议方法
            this.ensureBlankLine();
            this.emitBlockWithAccess(["static func modelCustomPropertyMapper() -> [String : Any]?"], () => {
            ...
            let arrayCount = 0;// 数组数量大于0才需要添加 modelContainerPropertyGenericClass 方法
            ...
            if (arrayCount > 0) {
                this.ensureBlankLine();
                this.emitBlockWithAccess("static func modelContainerPropertyGenericClass() -> [String : Any]?", () => {
                    this.emitLine("return [");
                    this.indent(() => {
                        this.forEachClassProperty(c, "none", (name, _jsonName, property) => {
                            if (property.type instanceof ArrayType) {
                                this.emitLine(`"`, name, `": `, this.swiftType(property.type.items, true), ".self,")
                            }
                        });
                    });
                    this.emitLine("]");
