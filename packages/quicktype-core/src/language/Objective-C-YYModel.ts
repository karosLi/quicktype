...
export const objcYYModelOptions = {
...
export class ObjectiveCYYModelTargetLanguage extends TargetLanguage {
    constructor() {
        super("Objective-C-YYModel", ["objc-yymodel", "objective-c-yymodel", "objectivec-yymodel"], "m");
    }

    protected getOptions(): Option<any>[] {
        return [
            objcYYModelOptions.justTypes,
            objcYYModelOptions.classPrefix,
            objcYYModelOptions.features,
            objcYYModelOptions.extraComments,
            objcYYModelOptions.marshallingFunctions
        ];
    }
    ...
    ): ObjectiveCYYModelRenderer {
        return new ObjectiveCYYModelRenderer(this, renderContext, getOptionValues(objcYYModelOptions, untypedOptionValues));
    }
}
...
export class ObjectiveCYYModelRenderer extends ConvenienceRenderer {
    ...
    constructor(
        ...
        renderContext: RenderContext,
        private readonly _options: OptionValues<typeof objcYYModelOptions>
    ) {
    ...
    protected emitPropertyAssignment(_propertyName: Name, _jsonName: string, _propertyType: Type) {
    ...
    private emitModelContainerPropertyGenericClass(propertyName: Name, propertyType: Type) {
        const name = ["", propertyName];// 转成字符串
        return matchType(
            propertyType,
            _anyType => [],
            _nullType => [],
            ...
            arrayType => [this.emitLine(`@"`, name, `"`, ": ", "[", this.emitModelContainerPropertyGenericClassType(arrayType, name), " class]", ",")],
            _classType => [],
            _mapType => [],
            _enumType => [],
            _unionType => [],
    ...
    private emitModelContainerPropertyGenericClassType(t: Type, typed: Sourcelike): Sourcelike {
        return matchType<Sourcelike>(
            t,
            _anyType => typed,
            _nullType => typed,
            ...
            _doubleType => typed,
            _stringType => typed,
            arrayType => ["", this.emitModelContainerPropertyGenericClassType(arrayType.items, "x")],
            classType => ["", this.nameForNamedType(classType)],
            mapType => ["", this.emitModelContainerPropertyGenericClassType(mapType.values, "x")],
            enumType => ["", this.nameForNamedType(enumType)],
            unionType => {
                const nullable = nullableFromUnion(unionType);
                return nullable !== null ? this.emitModelContainerPropertyGenericClassType(nullable, typed) : typed;
            }
        );
    }

    protected emitPrivateClassInterface(_: ClassType, _name: Name): void {
    ...
    private emitTopLevelFunctionDeclarations(_: Type, _name: Name): void {
    ...
    private emitClassImplementation(t: ClassType, className: Name): void {
        ...
        if (!this._options.justTypes) {
            ...
            this.ensureBlankLine();
            this.emitMethod("+ (NSDictionary *)modelCustomPropertyMapper", () => {
            ...
            this.forEachClassProperty(t, "none", (_name, _jsonName, property) => {
            ...
            if (arrayCount > 0) {
                this.ensureBlankLine();
                this.emitMethod("+ (NSDictionary *)modelContainerPropertyGenericClass", () => {
                    this.emitLine("return @{");
                    this.indent(() => {
                        this.forEachClassProperty(t, "none", (name, _jsonName, property) => {
                            if (property.type instanceof ArrayType) {
                                this.emitModelContainerPropertyGenericClass(name, property.type)
                            }
                        });
                    });
                    this.emitLine("};");
